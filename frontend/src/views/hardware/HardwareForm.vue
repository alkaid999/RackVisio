<template>
  <Dialog
    :model-value="visible"
    :title="isEdit ? '编辑硬件' : '新建硬件'"
    class="max-w-2xl"
    @update:model-value="(v) => emit('update:visible', v)"
  >
    <Form ref="formRef" :model="form" :rules="rules">
      <div class="grid grid-cols-2 gap-4">
        <FormItem name="type_id" label="硬件类型">
      ...
      <FormItem name="name" label="名称">
        <Input v-model="form.name" placeholder="如：32GB DDR4 ECC 内存条" />
      </FormItem>
      <div class="grid grid-cols-2 gap-4">
        <FormItem name="brand" label="品牌">
          <Input v-model="form.brand" placeholder="如：Intel / H3C / 希捷（选填）" />
        </FormItem>
        <FormItem name="sn" label="SN 号 / 编号">
          <Input v-model="form.sn" placeholder="独立编号（选填，非空时唯一）" />
        </FormItem>
      </div>
      <div class="grid grid-cols-2 gap-4">
        <FormItem name="spec" label="规格 / 型号">
          <Input v-model="form.spec" placeholder="如：32GB DDR4-3200 ECC（选填）" />
        </FormItem>
      </div>
      ...
      <p class="text-xs text-muted-foreground">每个硬件均为独立管理单元（一行 = 一件实物），建档后在库可分配至设备。</p>
    </Form>
...
const rules = {
  type_id: [{ required: true, message: '请选择硬件类型', trigger: 'change' }],
