<template>
  <div class="rack-panel">
    <!-- 头部：页面级标题 + 操作行（本组件仅由全局「机柜列表」路由页使用，不再内嵌到其它页） -->
    <div class="panel-head">
      ...
      <div class="panel-head-actions">
        ...
        <Button v-if="canEdit" variant="outline" @click="openBatchCreate"><Layers class="h-4 w-4" />批量新增</Button>
    ...
    <!-- 批量新增机柜弹窗 -->
    <RackBatchCreate v-model:visible="batchCreateVisible" :rooms="rooms" @saved="afterMutate" />
...
// Props:
// - initialRoomId: 跳转预筛机房（用于 ?room=<id> 跳转后预选，下拉仍可选，不锁定）。
// - filterKey: 持久化键，缺省 'RackList'。
const props = defineProps({
  initialRoomId: { type: [String, Number], default: '' },
  filterKey: { type: String, default: '' },
})
// 任意增删改后触发，供父级（如全局机柜列表页）感知刷新。
...
// 筛选持久化：按派生键存 sessionStorage，离开返回保留筛选；logout 统一清空。
...
onMounted(async () => {
  ...
  // 预筛优先级：跳转预筛（?room）> 持久化恢复（恢复由 usePersistentFilter 内部完成）。
  if (props.initialRoomId) {
    filter.roomId = String(props.initialRoomId)
  }
  load()
})
</script>
