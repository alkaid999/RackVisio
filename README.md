...
| **资产管理** | 统一管理耗材与硬件两大类资产（原「耗材管理」升级为「资产管理」） |
| **硬件管理** | 主板 / CPU / 内存 / 硬盘 / 阵列卡 / 网卡 / 电源等硬件，**每件独立台账**（SN 唯一、品牌字段）；自定义类型与分类；生命周期（新增/分配/回收/报废）全程留痕 |
| **设备硬件** | 设备详情页「设备硬件」卡片：从硬件管理选「在库」硬件分配至设备（一对一），回收后回库可再分配 |
