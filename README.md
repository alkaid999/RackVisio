...
- 基础设施管理（配线架 / ODF 配线架 / 其他设施，占 U 位但不计入资产统计、不建接口）
- 2D 机柜视图（U 位矩阵）+ 3D 机房 / 机柜视图（Three.js 第一人称视角）
- 机房平面拓扑内嵌视图（直观查看机柜排布与占用）
- 仪表盘统计（机柜使用率、设备类型 / 状态分布）
- 机房 / 机柜平面拓扑 **导出为 draw.io（`.drawio`）**，可在 diagrams.net 中继续编辑（浏览器端完成，无需后端）
- 浅色 / 深色主题切换（shadcn-vue 令牌体系，支持跟随系统）
- 机房 / 机柜 / 设备列表 **Excel 导入与导出**（批量录入与 U 位明细导出，浏览器端完成）
- 全局搜索（跨机房 / 机柜 / 设备快速定位）
- 操作审计日志（记录操作人、时间、模块与对象，覆盖机房/机柜/设备/接口/链路/上架/账号/耗材；可读名称展示）
- 登录鉴权（JWT）+ 基于角色的权限控制（RBAC），登录页默认密码智能提示
...
代码在构建时打进镜像，因此更新部署 = **先拉取最新源码，再重新构建并启动**；整个过程**无需删除数据库卷、也无需从备份恢复**，原有数据原样保留：
